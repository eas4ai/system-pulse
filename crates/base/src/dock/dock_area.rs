                ...
                ContainerPlan::Split {
                    ...
                    if previous != children || adopted != sizes {
                        state.update(cx, |state, cx| {
                            ...
                            //
                            if self.panel_policy == PanelPolicy::Separate {
                                // Separate panels retain absolute widths and
                                // collapsed header heights. Their constrained
                                // layout allocates any remaining space.
                                state.adopt_sizes(&sizes, cx);
                            } else {
                                // Tabbed layouts restore shares of the current
                                // container, which may differ from the window
                                // that saved these pixel sizes.
                                state.adopt_sizes(
                                    &scale_sizes_to(state.container_size(), &sizes),
                                    cx,
                                );
                            }
                        });
...
mod geometry_runtime_tests {
    ...
    #[gpui::test]
    fn separate_reconcile_keeps_explicit_slot_sizes_absolute(cx: &mut TestAppContext) {
        cx.update(|cx| {
            let _ = crate::Theme::global_mut(cx);
        });
        let (area, cx) =
            cx.add_window_view(|window, cx| DockArea::new("absolute-sizes", None, window, cx));
        for axis in [Axis::Horizontal, Axis::Vertical] {
            cx.update(|window, cx| {
                let panels: Vec<_> = ["absolute-alpha", "absolute-beta"]
                    .into_iter()
                    .map(|name| {
                ...
                area.update(cx, |area, cx| {
                    ...
                    area.set_center(
                        layout
                            .child(DockLayout::tabs().panel(panels[0].clone()), Some(px(420.)))
                            .child(DockLayout::tabs().panel(panels[1].clone()), Some(px(420.))),
                        window,
                        cx,
                    );
                });
            ...
            cx.update(|window, cx| {
                area.update(cx, |area, cx| {
                    let split = area.center.root().id();
                    let sizes = match axis {
                        Axis::Horizontal => vec![Some(px(360.)), Some(px(440.))],
                        Axis::Vertical => vec![Some(px(36.)), Some(px(280.))],
                    };
                    let expected: Vec<_> = sizes.iter().copied().flatten().collect();
                    let container = area.splits[&split].entity.read(cx).container_size();
                    assert!(container > px(0.));
                    assert_ne!(container, expected.iter().copied().sum::<Pixels>());
                    // An explicit tree edit must retain pixel dimensions, even
                    // when a previous frame measured a different container size.
                    area.center.set_sizes(split, sizes);
                    area.reconcile(window, cx);
                    assert_eq!(area.splits[&split].entity.read(cx).sizes(), &expected);
                });
            });
        }
    }
    ...
    fn refresh_repeatedly_restores_expanded_preferences_and_keeps_hidden_identity(
        ...
        for _ in 0..3 {
            cx.update(|window, cx| {
                ...
                assert_eq!(dock.splits[&split].sizes[0], Some(px(36.)));
                assert_eq!(
            ...
            cx.update(|window, cx| {
                area.update(cx, |area, cx| {
                    area.set_locked(!area.is_locked(), window, cx);
                    assert_eq!(area.splits[&split].entity.read(cx).sizes()[0], px(36.));
                });
            });
            assert_eq!(
