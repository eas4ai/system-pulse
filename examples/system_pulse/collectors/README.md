...
| Interface RX/TX / totals | `/sys/class/net/<name>/statistics/{rx,tx}_bytes` | Bytes/s and cumulative bytes; failures do not become zero. Stable physical path plus MAC, MAC-only, or interface-name fallback; hardware siblings cannot swap identities when one disappears. Includes real virtual interfaces. |
