...
5. Pending: fix native held-key input starvation, then full independent host/native acceptance and final review.
...
The collector attribution follow-up is committed through `bbfc24905d5566d37e655be2412b105e9edab218`; spec re-review passed and quality review is running. App/model integration passed both reviews, but the longer native probe has now established a concrete held-key input starvation defect. One source implementer at a time; independent spec then quality review follows each coherent task. Root owns execution documentation and independent acceptance preparation.
