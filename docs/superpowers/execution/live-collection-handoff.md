...
5. **In progress:** fix and review native held-key input starvation.
6. Pending: full independent host/native acceptance and final review.
...
The collector attribution follow-up is committed through `bbfc24905d5566d37e655be2412b105e9edab218`; both spec and quality reviews passed. App/model integration passed both reviews, but the longer native probe has now established a concrete held-key input starvation defect. One source implementer at a time; independent spec then quality review follows each coherent task. Root owns execution documentation and independent acceptance preparation.
...
Task 2b now addresses a confirmed native defect. Each process navigation key triggers a synchronous full layout while X11 drains queued input; foreground live delivery waits for that queue. Ordinary Up held for three seconds at 25 Hz caused over fourteen seconds of stale accepted data. The collector continued normally, and delivery recovered when input drained. Use one pending next-frame repaint per owning view while retaining every selection/scroll movement. Source plan and `native-freeze.md` record the task; complete report is `/tmp/system-pulse-freeze-investigation/REPORT.md`. Attribution reviews passed, and the app worker now has a narrow pending-frame fix with three red-first burst regressions. Its 44 app tests, 24 model tests, scoped strict Clippy/formatting/build pass. Native original-size held Up (3 seconds, 25 Hz) and an exactly 64-key burst pass with maximum accepted ages of 1.359 and 1.430 seconds. Horizontal/outer held checks are being completed before source commit and independent reviews. Candidate binary hash and exact artifacts are in source `native-freeze.md`. No final acceptance or completion is claimed.
