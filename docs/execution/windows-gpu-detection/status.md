...
Final accessibility candidate: e5797889fa2b297d2eb4d9441cf8fc665e69c423. All 22 screen tests and application Clippy passed; rustfmt check first requested a line wrap, which was applied, and the subsequent check passed. Windows build/package runs from `/home/shawn/workspace2/.system-pulse-test-tmp/build_windows_gpu_accessibility.py` into `windows-gpu-final-e5797889/`. The exact-source Mac tests/Clippy run uses `check_windows_gpu_macos_accessibility.py` and retains `windows-gpu-macos-preservation-e5797889/`. No new production edits are planned pending native label recapture.
