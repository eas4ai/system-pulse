...
The developer has confirmed the direction: the normal app displays real system readings; simulation belongs in tests. This draft makes the acceptance conditions explicit. It is not yet an Agreed Cairn contract. The existing simulation observations remain an accurate record of the defect's starting point.

## Scope and metric contract

This work replaces the fixture producer, discovery, process rows, and demonstration meter scaling in System Pulse. Linux is the first host-verification platform, following the [product specification](/home/shawn/workspace2/task-manager/docs/feature-spec-dockable-system-monitor.md:13). Cross-platform system collection uses the pinned sysinfo dependency where supported. Native macOS/Windows acceptance and their additional GPU adapters retain separate evidence needs; Linux success is not a claim about them.

The required monitor families and fields come from the [product's monitor definitions](/home/shawn/workspace2/task-manager/docs/feature-spec-dockable-system-monitor.md:41):
...
## Requirements and proposed falsifiers
...
Mechanism: proposed `live-runtime-source` check for production fixture dependencies, collector integration tests, and a native host comparison captured by `native-live-readings`.
Current evidence: [fixture formula](../../examples/system_pulse/src/fixture.rs:76), [runtime feed](../../examples/system_pulse/src/workspace.rs:404), [missing collector dependency](../../examples/system_pulse/Cargo.toml:8).
...
Mechanism: proposed `collector-discovery` tests with reordered, added, removed, unsupported, and permission-denied devices; native comparison with enumerated host devices and the capability report.
Current evidence: [fixed catalog](../../examples/system_pulse/src/fixture.rs:11).
...
Mechanism: proposed `live-process-table` tests for empty snapshots, insertion/removal, sorting, and PID reuse; native launch/exit of a harmless child process with PID/name comparison against the OS.
Current evidence: [generated rows and fixed navigation](../../examples/system_pulse/src/panel.rs:247); the reference's truncation is also unsuitable: [system_monitor](../../examples/system_monitor/src/main.rs:128).
...
Mechanism: proposed `collector-deltas` tests with known byte/CPU counters at unequal intervals, first samples, resets, and failed reads; native rate comparison using independently sampled counters.
Current evidence: [synthetic timestamps](../../examples/system_pulse/src/fixture.rs:91); the existing sample model rejects [non-finite values](../../examples/system_pulse/model/src/readings.rs:32).
...
Mechanism: proposed `physical-meter-scales` tests covering percentage, capacity, rate, temperature, counter, and compound summaries, followed by native numeric/chart comparisons.
Current evidence: [percentage clamping](../../examples/system_pulse/src/meters.rs:29), [untyped meter choices](../../examples/system_pulse/model/src/presentation.rs:7); intended kinds: [product matrix](/home/shawn/workspace2/task-manager/docs/feature-spec-dockable-system-monitor.md:97).
...
Mechanism: proposed `collector-availability` tests for individual sensor failure, whole-backend failure, recovery, and delayed snapshots; native permission/unsupported-backend inspection where reproducible.
Current evidence: [existing three-state reading model](../../examples/system_pulse/model/src/readings.rs:6), [compact formatter](../../examples/system_pulse/src/meters.rs:5). The implementation may model warm-up through explicit unavailable metadata rather than adding a redundant state.
...
Mechanism: proposed `live-identity-persistence` tests and native mixed-state restart with device absence/reappearance; migration tests retain the original fixture file and prove that fixture IDs are never assigned to physical devices by position.
Current evidence: [fixture-only validator](../../examples/system_pulse/src/workspace.rs:94), [fixture storage path](../../examples/system_pulse/src/storage.rs:12), [existing invalid-input retention](../../examples/system_pulse/model/src/persistence.rs:18).
...
Mechanism: proposed `collector-scheduling` test with a controlled slow backend and native responsiveness replay while collection runs.
Current evidence: the reference performs refresh in [the UI update](../../examples/system_monitor/src/main.rs:300); current app feed ownership is [WorkspaceView](../../examples/system_pulse/src/workspace.rs:322).
...
Mechanism: proposed `collector-lifecycle` tests for interval changes, backpressure, device churn, and shutdown; support the product's 0.5/1/2/5-second values with a one-second default.
Current evidence: [one existing timer](../../examples/system_pulse/src/workspace.rs:322), [per-series history without key eviction](../../examples/system_pulse/model/src/readings.rs:44); intended intervals: [product spec](/home/shawn/workspace2/task-manager/docs/feature-spec-dockable-system-monitor.md:205).
...
Current evidence: [approved workspace contract](/home/shawn/workspace2/task-manager/docs/superpowers/specs/2026-09-04-workspace-visibility-design.md), [verified baseline](../../examples/system_pulse/NATIVE_ACCEPTANCE.md).
...
Mechanism: proposed `live-runtime-source` check and native accessibility-tree inspection; tests retain deterministic injection through a test-only construction path.
Current evidence: [runtime toolbar](../../examples/system_pulse/src/workspace.rs:672), [entry title](../../examples/system_pulse/src/main.rs:21), [fixture module](../../examples/system_pulse/src/lib.rs:2).
...
Mechanism: proposed `collector-capability-coverage` check joins the required metric matrix to source/permission probes and adapter tests; an unimplemented accessible field fails the check. Unsupported attribution and denied permissions require recorded source evidence, not an adapter's absence.
Current evidence: the broader [product field list](/home/shawn/workspace2/task-manager/docs/feature-spec-dockable-system-monitor.md:41) exceeds the [fixture catalog](../../examples/system_pulse/src/fixture.rs:11).
...
Mechanism: proposed `native-live-readings` verifier records the source, formula, units, monotonic sample window, raw counters, and comparison bound before capture; verifier regression cases intentionally inject each mismatch and require failure.
Current evidence: [existing acceptance](../../examples/system_pulse/NATIVE_ACCEPTANCE.md) proves fixture interactions and supplies no host-accuracy comparison.
...
## Preparation for the first commitment

Proposed slug: `real-system-readings`. It will name LIVE-001 through LIVE-013 after their text and falsifiers are confirmed. The observed READ/VIEW/STATE requirements describe the starting point; fixture-specific observations will not be promoted into the target contract.

The mechanism names above are proposals, not existing tests or passing evidence. The implementation plan will give them executable commands and declared inputs in `.cairn/mechanisms/` before source changes. Collector units, snapshot delivery, typed rendering, dynamic identity/state integration, and native host verification are the implementation boundaries. Existing model and native tests remain preservation constraints.
