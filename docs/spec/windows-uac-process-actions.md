...
Mechanism: controlled disposable targets on an interactive Windows desktop; record source commit, binary hash, OS/toolchain, token elevation, requested PID/creation identity, exact action and observed outcome. Cover cooperative and refusing GUI targets, a windowless target, and ordinary/elevated Force Quit. Cover an administrator's consent prompt and cancellation. On 2026-09-10 the developer explicitly limited elevation acceptance to the administrator-account workflow; standard-user administrator-credential entry is outside this release commitment. Keep every harness-owned target under cleanup and do not target system-critical processes. Run native Windows tests/build and local package verification, plus applicable Linux/macOS preservation checks, before any separately authorized hosted build.
