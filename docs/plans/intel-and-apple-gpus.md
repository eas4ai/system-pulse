...
Task 4's first implementation is committed at `c014522d`. Independent [specification review](../execution/intel-and-apple-gpus/gpu-acceptance-spec-review.md) recorded nine findings, each before its correction. The original implementer is correcting them, including the [bounded native Intel workload](../decisions/verify-intel-gpu-load-with-a-bounded-native-vulkan-helper.md). All nine remain open until independent re-review of the committed corrections; quality review follows specification approval. The full Linux preservation run passed 847 automated tests but failed the native freshness check. A subsequent uninstrumented focused run also failed. The [retained failures and diagnostic](../execution/intel-and-apple-gpus/gpu-linux-preservation-failure.md) remain an unresolved Task 5 preservation obligation.
