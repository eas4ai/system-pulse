...
Task 2, Apple Silicon collection, is the single item currently in progress. Candidate `a7518cbe` passed independent [specification review](../execution/intel-and-apple-gpus/apple-spec-review.md), but [quality finding F1](../execution/intel-and-apple-gpus/apple-quality-review.md) requires a per-capture autorelease pool. The original implementer is correcting it; repeat native verification and independent review remain open. Source `df2300f2` passed 96 Linux and 44 native Mac tests, strict Clippy and formatting on both platforms, and a bounded native load capture, but a subsequent runtime diagnostic exposed the missing pool. The [implementation record](../execution/intel-and-apple-gpus/apple-implementation.md) links original commands, hashes and observations. Native smoke execution does not satisfy the later independent accuracy or UI acceptance. Task 1 closed at collector commit `507a12c0` after independent [specification](../execution/intel-and-apple-gpus/intel-spec-review.md) and [quality](../execution/intel-and-apple-gpus/intel-quality-review.md) reviews. A task remains open until its code and required verification/reviews are complete. Native hardware evidence is separate from deterministic adapter tests.
