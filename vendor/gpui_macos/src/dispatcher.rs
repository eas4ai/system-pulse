...
extern "C" fn trampoline(context: *mut c_void) {
    // Every invocation needs a pool on its own thread, including pending and
    // canceled tasks. Retained native objects and owned task results survive it.
...
#[cfg(test)]
#[path = "dispatcher_lifetime_tests.rs"]
mod lifetime_tests;
