...
unsafe fn get_process_infos(process: &mut ProcessInner, refresh_kind: ProcessRefreshKind) -> bool {
    // Retained names do not need KERN_PROCARGS2 unless a requested metadata
    // field needs refreshing. CPU, memory, identity and I/O refresh separately.
    if !process.name.is_empty()
...
mod process_argument_refresh_tests;
