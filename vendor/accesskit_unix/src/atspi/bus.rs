...
// AT-SPI cache signals carry one struct argument. zbus removes the outer
// structure as the argument list, so preserve the payload inside a tuple.
fn cache_signal_body<B>(body: &B) -> (&B,) {
    (body,)
}

#[cfg(test)]
#[path = "cache_signal_tests.rs"]
mod cache_signal_tests;
